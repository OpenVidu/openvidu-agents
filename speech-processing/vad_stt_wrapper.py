...
class VADTriggeredSTT(stt.STT):
    ...
    async def _recognize_impl(
        ...
        *,
        ...
        return await self._stt._recognize_impl(
...
class VADTriggeredSpeechStream(stt.SpeechStream):
    ...
    async def _run(self) -> None:
        ...
        logger.info(
            f"[VADTriggeredStream:{self._stream_id}] Starting _run() - coordinating VAD and STT"
        )
        ...
        logger.debug(
            f"[VADTriggeredStream:{self._stream_id}] Created underlying STT stream and VAD stream"
        )
        ...
        async def vad_task() -> None:
            ...
            logger.debug(
                f"[VADTriggeredStream:{self._stream_id}] vad_task started - listening for VAD events"
            )
            vad_event_count = 0
            ...
            async for ev in self._vad_stream:
                ...
                if ev.type == vad.VADEventType.START_OF_SPEECH:
                    ...
                    logger.debug(
                        f"[VADTriggeredStream:{self._stream_id}] Forwarded START_OF_SPEECH to event channel"
                    )
                    ...
                    if self._flush_delay > 0:
                        logger.debug(
                            f"[VADTriggeredStream:{self._stream_id}] Waiting {self._flush_delay}s before flush signal"
                        )
                        await asyncio.sleep(self._flush_delay)

                    logger.debug(
                        f"[VADTriggeredStream:{self._stream_id}] Setting flush_event to trigger STT flush"
                    )
                    flush_event.set()
                    ...
                    logger.debug(
                        f"[VADTriggeredStream:{self._stream_id}] Forwarded END_OF_SPEECH to event channel"
                    )
                    ...
                    logger.debug(
                        f"[VADTriggeredStream:{self._stream_id}] VAD event: {ev.type}"
                    )

            logger.debug(
                f"[VADTriggeredStream:{self._stream_id}] vad_task ended - processed {vad_event_count} events"
            )
        ...
        async def stt_forward_task() -> None:
            ...
            logger.debug(
                f"[VADTriggeredStream:{self._stream_id}] stt_forward_task started - listening for STT events"
            )
            stt_event_count = 0
            filtered_count = 0
            ...
            async for frame in self._input_ch:
                ...
                if isinstance(frame, rtc.AudioFrame):
                    ...
                    if flush_event.is_set():
                        ...
                        self._stt_stream.flush()
                        logger.debug(
                            f"[VADTriggeredStream:{self._stream_id}] flush() called on STT stream"
                        )
        ...
        logger.info(
            f"[VADTriggeredStream:{self._stream_id}] Starting 3 concurrent tasks: vad_task, stt_forward_task, audio_forward_task"
        )
        tasks = [
            asyncio.create_task(vad_task(), name=f"vad_task_{self._stream_id}"),
            asyncio.create_task(
                stt_forward_task(), name=f"stt_forward_task_{self._stream_id}"
            ),
            asyncio.create_task(
        ...
        try:
            ...
            logger.info(
                f"[VADTriggeredStream:{self._stream_id}] All tasks completed normally"
            )
        except Exception as e:
            logger.error(
                f"[VADTriggeredStream:{self._stream_id}] Task error: {e}", exc_info=True
            )
            raise
        finally:
            logger.debug(
                f"[VADTriggeredStream:{self._stream_id}] Cleaning up - cancelling tasks and closing streams"
            )
